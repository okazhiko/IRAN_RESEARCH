...
OUTPUT_PATH = BASE_DIR / "results" / RUN_DATE / f"{RUN_DATE}_jepx_system_lng.png"
...
def main() -> None:
    ...
    jepx_df = df[["time", "jepx_price"]].dropna().sort_values("time")
    lng_df = df[["time", "lng_price"]].dropna().sort_values("time")
